...
class OpeningTree:  
    def __init__(self, list_of_games):
        ...
        self.create_tree()
        self.game_opening = "Opening: " + self.list_of_games[0].lookup_meta_data("Opening") + "\nVariation: " + self.list_of_games[0].lookup_meta_data("Variation")
    ...
    def print_node(self, node, depth, current_depth, dot_file):
        if current_depth < depth: # not leaf node and not at max depth
            for child in node.get_children():
                dot_file.write('{} [label="{}", fillcolor="{}", style="filled"];\n'.format(str(id(node)), node.get_result(), node.get_color())) # write node
        ...
        if node.get_parent() is None: # if node is root node
            dot_file.write('{} [label="{}", fillcolor="{}", style="filled"];\n'.format(str(id(node)), self.game_opening, node.get_color())) # write root node
...
def main():
    ...
    for e in eco:
        ...
        save_tree_from_list_of_games(list_of_games, 8, "tree_{}".format(e))
    ...
    for opening in openings:
        list_of_games = database.get_games_with_opening(opening)
        save_tree_from_list_of_games(list_of_games, 8, "tree_{}".format(opening.replace(" ", "_")))
